    		...
    		<div class="col-md-4 col-md-offset-2 col-sm-5 col-sm-offset-1">
      		<label for="validationServer09">Taxi</label>
		  		<select id="taxi" name="taxi">
		  			...
		  			<option value="<%=t.getId()%>" <%if(t.getAutista()!=null && t.getAutista().getId().longValue() == u.getId().longValue()) {%> selected="selected" <%}%>><%=t.getMarca() %> <%=t.getModello() %>, <%=t.getTarga()%>, <%if (t.getAutista()==null){%> Non assegnato <%} else { %> Assegnato <%} %></option> <%} %>
